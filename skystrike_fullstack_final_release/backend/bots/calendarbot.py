...
async def build_order(ticker: str, contracts: int, dte: int, mode: str) -> Dict[str, Any]:
    """
    Use same-strike, different-expiration calendar spread to bet on IV expansion.
    """
    ...
    logger.info(f"[CALENDARBOT] {ticker} spread {short_strike}/{long_strike} exp {expiration} x{contracts}")
