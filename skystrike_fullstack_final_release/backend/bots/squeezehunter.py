...
async def build_order(ticker: str, contracts: int, dte: int, mode: str) -> Dict[str, Any]:
    """
    Enter tight OTM vertical before breakout using volatility squeeze logic.
    """
    ...
    logger.info(f"[SQUEEZEHUNTER] {ticker} spread {short_strike}/{long_strike} exp {expiration} x{contracts}")
